...
def run_virtual_keyboard():
    ...
    def press_key(key):
        ...
        try:
            ...
            # # If the keyboard window is active, try to switch to another window
            # if current_window and "Virtual Keyboard" in current_window.title:
            #     all_windows = gw.getWindowsWithTitle("")
            #     for w in all_windows:
            #         if w.title and "Virtual Keyboard" not in w.title:
            #             try:
            #                 w.activate()
            #                 time.sleep(0.05)
            #                 break
        ...
        # Send actual key to the currently focused window
        if key == "SPACE":
...
def run_overlay():
    ...
    def start_keyboard_thread():
        kb_thread = threading.Thread(target=run_virtual_keyboard, daemon=True)
        kb_thread.start()
